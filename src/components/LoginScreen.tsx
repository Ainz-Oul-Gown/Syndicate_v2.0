...
export function LoginScreen({ onLoginSuccess, isError, loadingText, deferredPrompt, setDeferredPrompt }: LoginScreenProps) {
  ...
  const customAuthCall = async (stableId: number, name: string | null, publicKeysPayload: any | null, isRegister: boolean) => {
    const { data: resData, error: fetchErr } = await supabaseClient.functions.invoke('auth-custom', {
      body: { stableId, name, publicKeysPayload: publicKeysPayload ? JSON.stringify(publicKeysPayload) : null, isRegister }
    });
  ...
  const handleSeedRegister = async () => {
    ...
    try {
      ...
      // 4. Save to Database
      const token = await customAuthCall(numericId, regName.trim(), publicKeysPayload, true);
  ...
  const handleTelegramOtpSubmit = async () => {
    ...
    try {
      ...
      if (telegramAction === 'register') {
        ...
        const token = await customAuthCall(stableId, telegramName.trim(), publicKeysPayload, true);await idbKeyval.set(`my_private_key_${stableId}`, rsaKeyPair.privateKey);
        await idbKeyval.set(`my_sign_key_${stableId}`, ecdsaKeyPair.privateKey);
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      ...
      if (emailAction === 'register') {
        ...
        const token = await customAuthCall(stableId, emailName.trim(), publicKeysPayload, true);await idbKeyval.set(`my_private_key_${stableId}`, rsaKeyPair.privateKey);
        await idbKeyval.set(`my_sign_key_${stableId}`, ecdsaKeyPair.privateKey);
  ...
  const handleRealGoogleSignIn = async () => {
    ...
    try {
      ...
      let stableId = getStableNumericId(user.uid); // use robust Google UID
      let userProfile = null;

      if (googleAction === 'login') {
        // First try to find profile by Google UID stableId
        const { data: profileByUid } = await supabaseClient
          .from('users')
          .select('*')
          .eq('tg_id', stableId)
          .maybeSingle();
        ...
        if (profileByUid) {
          userProfile = profileByUid;
        } else {
          // Fallback: Try legacy Google Email stableId (simulated accounts)
          const legacyStableId = getStableNumericId(accountEmail);
          const { data: profileByEmail } = await supabaseClient
            .from('users')
            .select('*')
            .eq('tg_id', legacyStableId)
            .maybeSingle();

          if (profileByEmail) {
            userProfile = profileByEmail;
            stableId = legacyStableId; // Update stableId to the legacy email-based ID
          }
        }
        ...
        const keysPayload = JSON.parse(userProfile.public_key || '{}');
        if (keysPayload.vault) {
          // Attempt 1: Try to decrypt using user.uid derived key
          const googleDerivedAesUid = await deriveAesKeyFromSeed(`google-auth-key-derivation-salt-${user.uid}`);
          let decryptedKeys = await decryptVault(googleDerivedAesUid, keysPayload.vault);

          // Attempt 2 Fallback: Try to decrypt using accountEmail derived key (legacy)
          if (!decryptedKeys && accountEmail) {
            console.log("🔒 Attempting legacy Google decryption via email salt...");
            const googleDerivedAesEmail = await deriveAesKeyFromSeed(`google-auth-key-derivation-salt-${accountEmail}`);
            decryptedKeys = await decryptVault(googleDerivedAesEmail, keysPayload.vault);
          }
          ...
          if (decryptedKeys) {
            ...
            if (!rsaJwk || !ecdsaJwk) {
              hapticImpact("error");
              setErrorMessage('Неверный формат расшифрованных ключей в сейфе Google.');
              setIsSubmitting(false);
              return;
            }
            ...
            hapticImpact("error");
            setErrorMessage('Не удалось дешифровать крипто-сейф вашего Google аккаунта.');
            setIsSubmitting(false);
            return;
          }
        }
